    ...
    return ( 
        <div className = "w-screen h-screen flex flex-row flex-wrap justify-start items-center bg-zinc-200">
            {!loaded ?(
                ...
                items.map((item) => {
                    return <Item key ={item.id} id={item.id} name={item.name} img1URL={item.img1URL} img2URL = {item.img2URL} img3URL={item.img3URL} price={item.price} artists={item.artists} />
                })
            )}
