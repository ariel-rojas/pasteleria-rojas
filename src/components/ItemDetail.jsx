...
const ItemDetail = ({item}) => {
    ...
    return ( 
        ...
        <div className = "flex flex-col w-[400px] h-[400px] m-[20px] text-center bg-center bg-no-repeat bg-contain" style={{backgroundImage: `url(${item[0].img1URL})`}}>
        </div>
        <div className = "flex flex-col w-[400px] h-[400px] m-[20px] text-center">
            ...
            <div className="w-full h-[300px] flex flex-col justify-center text-center">
                <h3 className='min-h-[4rem]'>{item[0].name}</h3>
                <p>Artista: {item[0].artists}</p>
                <p>Precio: ${item[0].price}</p>
                <p>Lanzamiento: {item[0].year}</p>
